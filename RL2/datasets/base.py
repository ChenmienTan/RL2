...
def get_dataloader(dataset, batch_size):
    return StatefulDataLoader(
        dataset, batch_size, shuffle=True, drop_last=True, collate_fn=dataset.collate_fn
    )


def get_tensor_dict(states, actions, action_mask, max_length=None, rm=False):
    ...
    if rm:
        tensor_dict["action_mask"] = torch.LongTensor((len(states) - 1) * [0] + [1])
    else:
...
def pack_tensor_dicts(tensor_dicts):
    return {
        k: pad_sequence([tensor_dict[k] for tensor_dict in tensor_dicts], True)
...
class BaseDataset(Dataset):
    ...
    def tokenize_prompt_response(self, prompt, response, rm=False):
