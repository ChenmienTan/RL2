...
# Multi-Agent Code Review Training Script
# Reviewer identifies issues, Author fixes them
...
# Dataset: Code snippets that need review
# Format: {"code": "def foo():\n    return bar"}
TRAIN_PATH="${TRAIN_PATH:-train@your_org/CodeReview}"
TEST_PATH="${TEST_PATH:-test@your_org/CodeReview}"
...
PROJECT="${PROJECT:-CodeReview}"
EXPERIMENT_NAME="${EXPERIMENT_NAME:-multi_agent_code_review}"
...
torchrun \
    ...
    rollout.env_path=envs/multi_agent_code_review.py \
